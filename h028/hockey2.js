...
let hockeyGame = {
     ...
     */
    fillfield: function (fmaxX, fmaxY, b_z, s_z) {
        this.fmaxX = fmaxX;
        this.fmaxY = fmaxY;
        this.s_z = s_z;
        this.b_z = b_z;
        ...
        enumfields(function (i, j) {
            let cost = 0;
            if (j <= midY)
                cost = 100 * (-midY + Math.max(j, Math.min(Math.abs(i - midX - 1), Math.abs(i - midX), Math.abs(i - midX + 1))));
            else
                cost = 100 * (midY - Math.max(fmaxY - 1 - j, Math.min(Math.abs(i - midX - 1), Math.abs(i - midX), Math.abs(i - midX + 1))));
    ...
    calc: function (depth, color) {
        let BestTurn = null;
        let MaxDepth = 8, bt,
            [MaxScore, Score] = color ? [this.minRed-200, this.maxBlue + 100] : [this.maxBlue+200 , this.minRed-100], x,
            i = null;
        this.curcolor = color;
...
function createField() {
    ...
    hockeyGame.fillfield(fmaxX, fmaxY,20,45);
    let r_len=hockeyGame.s_z;
    ...
    //place border
    let b_z = hockeyGame.b_z, s_z = r_len, midX = fmaxX >> 1, midY = fmaxY >> 1;
    ...
    for (let i = 0; i < fmaxX; i++) for (let j = 0; j < fmaxY; j++) {
        ...
        if (Math.abs(midX - i) < 2 && j === 0) {
            a["class"] = `box col0`;
            a['r']=7;
        }
        if (Math.abs(midX - i) < 2 && j === fmaxY - 1) {
            a["class"] = `box col1`;
            a['r']=7;
        }
...
function turn(x) {
    var s;
    if (!hockeyGame.curpos.lines[x] || !hockeyGame.curpos.lines[x].can) return;
    let i0 = hockeyGame.curpos.x, j0 = hockeyGame.curpos.y;

    hockeyGame.mk_turn(x);
    //drawturn(x,curcolor==0?'red':'blue');
    // svg field
    let b_z = hockeyGame.b_z, s_z = hockeyGame.s_z;
    let disp = hockeyGame.disp[x],
...
function backturn(){
    ...
    let lastturn=hockeyGame.turns[hockeyGame.turns.length-1];
    if(lastturn.color==-1) return;
    let svg = sh.$('svg-turns');
    if(lastturn.color==1-c) {
        while (lastturn.color == 1 - c) {
            hockeyGame.back_turn();
            svg.removeChild(svg.lastChild);
            lastturn = hockeyGame.turns[hockeyGame.turns.length - 1];
        }
    }
    ...
    let i0=hockeyGame.curpos.x, j0=hockeyGame.curpos.y, b_z=20, s_z=45;
    sh.attr(sh.$('svg-ball').children[0], {
        'cx': `${b_z + (i0 - 1) * s_z}`,
        'cy': `${b_z + (j0 - 1) * s_z}`
    });
...
function xturn(x) {
    var c = hockeyGame.curcolor;
    if (hockeyGame.curpos.cost <= hockeyGame.minRed) {
        alert('Red player win!');
        return;
    } else if (hockeyGame.curpos.cost >= hockeyGame.maxBlue) {
        alert('Blue player win!');
        return;
    }
    if (x < 0) {
        x = [0]
        while ((x[0] !== null) && c == hockeyGame.curcolor
            && (!(hockeyGame.curpos.cost <= hockeyGame.minRed || hockeyGame.curpos.cost >= hockeyGame.maxBlue))
        ) {
