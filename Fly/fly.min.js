...
(function () {
    ...
    function the_fly(param) {
        ...
        this.rules = {walk: {cadr: [1, 2, 3, 4, 5]}, stay: {cadr: [6, 7, 8]}, fly: {cadr: [9, 10]}, hidden: {}};
        if (param) {
            for (a in param) {
                this[a] = param[a]
            }
        ...
        for (var i in this.rules) {
            for (var j in this._default) {
                if (!this.rules[i][j]) {
                    this.rules[i][j] = this._default[j]
                }
            }
        }
        ...
        this.nextstep = function (state, event) {
            ...
            if (!rule[event] || rule[event].length == 0) {
                return false
            }
        ...
        function drawKadr(ctx, _img, kadr) {
            ...
            ctx.drawImage(_img, 0, kadr * this.obj.fFheight, this.obj.fwidth, this.obj.fheight, this.obj._x, this.obj._y, this.obj.fwidth, this.obj.fheight);
            ctx.restore()
        }
        ...
        this.draw = function () {
                ...
                if (!((past.x != cursor.x || past.y != cursor.y) && this.nextstep(1, "_mousemove"))) {
                    if (!(this._cnt-- <= 0 && this.nextstep(1, "_timeout"))) {
                        ...
                        if (rule.kadr) {
                            ...
                            if (++this.kadrnum >= rule.kadr.length) {
                                this.kadrnum = 0
                            }
                        }
                    }
            ...
            if (this.element && this.element.getContext) {
                if (!(this.coord.x < engine.scroll.x - this.obj.width / 2
                    || this.coord.x > -this.obj.width  + engine.window.x + engine.scroll.x
    ...
    engine = {
        scroll: {x: 0, y: 0}, calc_Bounds: function () {
            engine.window = {
            ...
            engine.scroll = {
                x: (document.documentElement && document.documentElement.scrollLeft) || (document.body && document.body.scrollLeft),
                y: (document.documentElement && document.documentElement.scrollTop) || (document.body && document.body.scrollTop)
            }
        }, _handlers: {}, _curhandle: "0", set: function (x) {
            if (!x) {
                x = "0"
            }
            this._curhandle = x;
            ...
            return this
        }, mousetgt: (window.execScript || window.opera) ? document : window, clearEv: function (e) {
            if (e.preventDefault) {
                e.preventDefault()
            }
            e.cancelBubble = true;
            ...
            this.add_event(this.mousetgt, "mousemove", function (e) {
    ...
    engine.init([{
        name: "cow",
        cnt: 2,
        obj: {fid: "cow_01", width: 87, height: 87, fwidth: 70, fheight: 51, fFheight: 50, _x: -35, _y: -25.5},
        _default: {},
        rules: {
            walk: {
                _mousemove: "stay",
            ...
            fly: {
                mintime: 20, maxtime: 40, _timeout: ["stay", "fly"], speed: 30, kadr: [0], init: function () {
                    this.angle = this.lookat({
                        x: Math.random() * engine.window.x + engine.scroll.x,
                        y: Math.random() * engine.window.y + engine.scroll.y
                    })
