...
Раскопал свой древний проект, минимально причесал и исправил ошибки, вроде...

Пока реализаван только `хоккей`
...
Формируется поле игры, каждая клетка которого содержит весь набор ходов из этой точки. Из за особенностей JS (объекты  -всегда ссылки) сначала формируется набор объектов-ходов, при этом ход из точки А в В и наоборот - это один и тот же ход, потом эти ходы раскидываются в массивы по нужным точкам поля. В `ходе` ставится отметка - он уже сделан или нет и в каком направлении.
Перебор возможных ходов делается из стартовой точки, выбирая все несделанные пока ходы. После того, как ход выбран, он помечается `сделанным`, стартовая точка перемещается в другой узел поля. На стек ходов помещается ход, в которых добавляется координаты бывшей стартовой позиции, цвет игрока. `Отмена` хода делается по логу ходов. У последнего в стеке лога убирается отметка и позиция мяча переносится в стартовую позицию.
